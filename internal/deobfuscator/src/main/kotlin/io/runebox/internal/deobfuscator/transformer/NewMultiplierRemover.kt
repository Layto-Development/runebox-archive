...
class NewMultiplierRemover : Transformer {
    ...
    private class MultiplierSolver(private val pool: ClassPool, private val multipliers: Multipliers) {
        ...
        private class MultiplierSolverInterpreter(private val multipliers: Multipliers) : Interpreter<MulValue>(ASM9) {
            ...
            override fun binaryOperation(insn: AbstractInsnNode, value1: MulValue, value2: MulValue) = MulValue.Binary(inter.binaryOperation(insn, value1.src, value2.src), value1, value2).also {
                when(insn.opcode) {
                    IMUL, LMUL -> {
                        val fieldMul = it.asFieldMul() ?: return@also
                        if(ldcMuls.add(fieldMul.ldc)) {
                            multipliers.unsolvedMultipliers.put(fieldMul.field.id, Multiplier.decoder(fieldMul.ldc.number))
                        }
                    }
            ...
            private fun distribute(putter: FieldInsnNode, value: MulValue.Binary) {
                if(value.isMultiply()) {
                    ...
                    if(fieldMul != null && fieldMuls.add(fieldMul.ldc)) {
                        multipliers.unsolvedMultipliers.remove(fieldMul.field.id, Multiplier.decoder(fieldMul.ldc.number))
    ...
    private class MultiplierRemover(private val pool: ClassPool, private val multipliers: MultiplierSolver.Multipliers) {
        ...
        private class MultiplierRemoverInterpreter : Interpreter<Expr>(ASM9) {
            ...
            override fun copyOperation(insn: AbstractInsnNode, value: Expr) = Expr.Var(inter.copyOperation(insn, value.src))

            override fun unaryOperation(insn: AbstractInsnNode, value: Expr) = inter.unaryOperation(insn, value.src)?.let { Expr.Var(it) }

            override fun naryOperation(insn: AbstractInsnNode, values: MutableList<out Expr>) = inter.naryOperation(insn, emptyList())?.let { Expr.Var(it) }
            ...
            override fun merge(value1: Expr, value2: Expr): Expr {
                if(value1 == value2) {
                    ...
                    if(value1.a == value2.a && value1.a is Expr.Ldc) {
                        return Expr.Mul(value1.src, value1.a, merge(value1.b, value2.b)).also { mulExprs[value1.insn] = it }
                    } else if(value1.b == value2.b && value1.b is Expr.Ldc) {
                        return Expr.Mul(value1.src, merge(value1.a, value2.a), value1.b).also { mulExprs[value1.insn] = it }
                    }
